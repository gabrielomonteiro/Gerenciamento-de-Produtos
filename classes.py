...
class Usuario:
    def __init__(self):
        self.df = pd.DataFrame(columns=['Usuário', 'Senha'])
        self.arquivo_login = 'login.xlsx'
        self.criar_tabela()

    def criar_tabela(self):
        # Cria a tabela de usuários se não existir
        if not os.path.exists(self.arquivo_login):
            self.df.to_excel(self.arquivo_login, index=False)

    def usuario_existe(self, usuario):
        # Verifica se o usuário já existe na tabela
        return any(self.df['Usuário'] == usuario)
